...
// Use special case of binary search
int BinSearch(int lo, int hi, const vector<int>& nums) {
  if (lo >= hi) return nums[lo];

  int mid = lo + (hi - lo) / 2;
  if (nums[mid] < nums[hi]) return BinSearch(lo, mid, nums);
  if (nums[mid] > nums[hi]) return BinSearch(mid + 1, hi, nums);
  return BinSearch(lo, hi - 1, nums);
}

int FindMin(vector<int>& nums) {
  int n = nums.size();
  return BinSearch(0, n - 1, nums);
}
