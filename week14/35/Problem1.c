...
char *intToRoman(int num)
{
    ...
    char *res = (char *)malloc(20 * sizeof(char)); // Enough size for the maximum value (3999)
    res[0] = '\0';                                 // Initialize result string

    // Traverse through the values and symbols arrays
    for (int i = 0; i < 13; i++)
    {
        while (num >= values[i])
        {
