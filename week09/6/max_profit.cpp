...
stock share for a period of N consecutive days. If a single share was bought on
day P and sold on day Q, where 0 ≤ P ≤ Q < N, then the profit of such
transaction is equal to A[Q] − A[P], provided that A[Q] ≥ A[P]. Otherwise, the
...
sold on day 5, a profit of 354 would occur because A[5] − A[4] = 21367 − 21013 =
354. Maximum possible profit was 356. It would occur if a share was bought on
day 1 and sold on day 5.
...
int solution(vector<int>& A) {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    int prev = A[i - 1];
    if (prev > cur) {
      diff = max(diff, prev - min_val);
      min_val = min(cur, min_val);
    }
    if (i == N - 1) {
      diff = max(diff, cur - min_val);
    }
  }
  return diff;
}
